...
def read_data(index_num):
	print("reading data...")
	data_path = '../data/cms/many/sum/out'+str(index_num)+'/0_sim_sum.txt'
	time,cell_pos_x,cell_pos_y,msd,msd_sem,module_ref_y,dt,traction,aflow,module_num,module_subk,module_engage_num,module_force = [],[],[],[],[],[],[],[],[],[],[],[],[]
	with open(data_path, 'r') as f: 
		for line in f:
			if 'time' in line:
				line = next(f)
				time = [float(i) for i in line.split() if i != '\n']
			if 'cell_pos_x' in line:
				line = next(f)
				cell_pos_x = [float(i) for i in line.split() if i != '\n']
			if 'cell_pos_y' in line:
				line = next(f)
				cell_pos_y = [float(i) for i in line.split() if i != '\n']
			if 'msd' in line:
				line = next(f)
				msd = [float(i) for i in line.split() if i != '\n']
			if 'msd sem' in line:
				line = next(f)
				msd_sem = [float(i) for i in line.split() if i != '\n']
			if 'dt' in line:
				line = next(f)
				dt = [float(i) for i in line.split() if i != '\n']
			if 'traction' in line:
				line = next(f)
				traction = [float(i) for i in line.split() if i != '\n']
			if 'aflow' in line:
				line = next(f)
				aflow = [float(i) for i in line.split() if i != '\n']
			if 'module num' in line:
				line = next(f)
				module_num = [float(i) for i in line.split() if i != '\n']
			if 'module ref y' in line:
				line = next(f)
				while(1):
					if '<' in line:
						break
					else:
						module_ref_y.append([float(i) for i in line.split() if i != '\n'])
						line = next(f)
			
			if 'module subk' in line:
				line = next(f)
				while(1):
					if '<' in line:
						break
					else:
						module_subk.append([float(i) for i in line.split() if i != '\n'])
						line = next(f)

			if 'module engage num' in line:
				line = next(f)
				while(1):
					if '<' in line:
						break
					else:
						module_engage_num.append([float(i) for i in line.split() if i != '\n'])
						line = next(f)
			
			if 'module force' in line:
				line = next(f)
				while(1):
					if '<' in line:
						break
					else:
						module_force.append([float(i) for i in line.split() if i != '\n'])
						line = next(f)
			

	return time,cell_pos_x,cell_pos_y,msd,msd_sem,module_ref_y,dt,traction,aflow,module_num,module_subk,module_engage_num,module_force
...
if __name__ == "__main__":
	...
	for i in range(len(curves)):
		...
		print((s,e))
		if i == 1:
			plt.plot(curves[i][0][s:e], curves[i][1][s:e], color = 'red')
		else:
